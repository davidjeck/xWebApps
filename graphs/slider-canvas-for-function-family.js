...
// this is a script named slider-canvas.js modified for use with function-family.html
// because the original had no way to set the min/max of a slider and it left too
// much space above and below the slider.  As it stands, it will only work for a
// SliderCanvas that contains exactly one slider.
...
function SliderCanvas(canvas) {
    ...
    this.graphics.font = "14px bold sans-serif";
    var sliders = [];
...
SliderCanvas.prototype.draw = function() {
    ...
    if (this.rightMargin == null) {  // signal to compute right margin and slider lengths
        ...
        for (i = 0; i < this.sliders.length; i++) {
            ...
            this.sliders[i].top = -5;   // FUDGED FOR USE WITH function-family.html -- only one slider, minimal border on top and bottom
